...
```{r poisson-aggregate-comparison, results = 'hide', cache=TRUE,message=FALSE,warning=FALSE}
n_sims <- 20
...
```{r full agg sim, cache=TRUE, results='hide',message=FALSE,warning=FALSE}
n_sims <- 20
m_values <- c(50,100,250,500)
no_methods <- 3 #ours, sum all, pz_estimator
...
```{r inhawkes-aggregation-comparison, message=FALSE,warning=FALSE}
...
```{r simulate inhom Hawkes, results='hide',eval=FALSE}
m <- 50
K <- 2
H <- 2
MuA <- array(0,c(K,K,H))
...
```{r functions for binned data,results='hide',eval=FALSE}
...
out <- bin_fun(test_events,m,max_Time = Time,window_size = 1)
sum(out) - nrow(alltimes)

```
```{r fit our model, results='hide',eval=FALSE}
B_start <- matrix(runif(K*K),K,K)
MuA_start <- array(runif(K*K*H),c(K,K,H))

tau_start = matrix(runif(m*K),nrow=m,ncol=K)
tau_start = tau_start/rowSums(tau_start)
