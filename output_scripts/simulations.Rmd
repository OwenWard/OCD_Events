...
2. We follow 
[Pensky and Zhang](https://projecteuclid.org/euclid.ejs/1550286096),
...
```{r initial fit link prediction homog Poisson, results='hide'}
train_time <- 75
...
### Link Prediction for Hawkes

We now want to repeat this simulation example for some
more expressive intensity functions, first the Hawkes process.

```{r link prediction hawkes sim data}
Time <- 200
inter_T <- 5
K <- 3
...
```{r hawkes link predict fit training,results='hide'}
#train_time <- 50

training_data_hawkes <- alltimes_hawkes[alltimes_hawkes[,3]<= train_time,]
test_data_hawkes <- alltimes_hawkes[alltimes_hawkes[,3]> train_time,]
...
out <- capture.output( training_fit_hawkes <-
                  online_estimator_eff_revised(training_data_hawkes,
                                               A, m,
                                               K,
                                               T = train_time,
                                               dT=0.1,
...
# pass these in and repeatedly update them
S_start <- training_fit_hawkes$S
tau_start <- training_fit_hawkes$tau
B_start <- training_fit_hawkes$B
Mu_start <- training_fit_hawkes$Mu
Pi_start <- training_fit_hawkes$Pi
lam_start <- training_fit_hawkes$lam

```



```{r hawkes link prediction sim data, results='hide',cache=TRUE}
time_windows <- seq(train_time,Time, by = 5)

test_data_hawkes <- as_tibble(test_data_hawkes)
colnames(test_data_hawkes) <- c("send","rec","Time")
...
for(i in 1:(length(time_windows)-1)) {
  ...
  data_batch <- test_data_hawkes %>% 
    filter(Time > start_time & Time < end_time)
  
  ### link predict for this data ###
  curr_z <- apply(tau_start,1,which.max)
  
  est <- Predict_Counts_Hawkes(startT = start_time,
                              finalT = end_time,
                              A, curr_z-1,
                              Mu_start,
                              B_start,
                              ...
                              lam_start)
  est <- est[-1,]
  colnames(est) <- c("send","rec","n_pred")
  est <- as_tibble(est)
  ...
  difference <- true_counts %>%
    full_join(est, by = c("send","rec")) %>%
    mutate(across(starts_with("n"),~replace_na(.x,0))) %>%
    mutate(diff = n - n_pred)
  ...
  ### then update the model params ####
  current_fit_hawkes <- online_estimator_eff_revised(as.matrix(data_batch),
                                                   A, m, K, T = end_time,
                                                   dT = 0.2,
                                                   lam = lam_start,
                                                   B_start = B_start ,
  ...
  # update these params ##
  S_start <- current_fit_hawkes$S
  tau_start <- current_fit_hawkes$tau
  B_start <- current_fit_hawkes$B
  Mu_start <- current_fit_hawkes$Mu
  Pi_start <- current_fit_hawkes$Pi
  lam_start <- current_fit_hawkes$lam
  #print(lam_start)
...
```{r hawkes link pred plot}
overall_pred %>%
  ggplot(aes(end,norm)) + geom_line()
```

What about repeating this over multiple fits for the same data?

```{r hawkes link pred multiple fits, cache=TRUE,results='hide'}
all_preds <- tibble()
...
for(sim in 1:n_sims) {
  ...
  all_preds <- bind_rows(all_preds,overall_pred)
...
```{r plot these predictions}
all_preds %>% ggplot(aes(end,norm)) +
  geom_smooth() + geom_point() +
  geom_line(aes(colour = as.factor(sim))) + theme(legend.position = "none")
```

### Link Prediction Inhom Hawkes

```{r sim data inhom hawkes }
...
B <- matrix(c(0.2,0.05,0.3,0.15),K,K,byrow = TRUE)
m <- 100
...
alltimes_hawkes <- sampleBlockHak_nonhomo(Time,A, Z, MuA, B,
                                                   window = 1, lam = 1.25)

```

```{r fit initial, results='hide'}
#train_time <- 50
...
fit_wind <- 0.25
Pi <- rep(1/K,K)
Mu_A <- array(runif(K*K*H),dim=c(K,K,H))
B <- matrix(runif(K*K),K,K)
tau <- matrix(runif(m*K),nrow=m,ncol=K)
tau <- tau/rowSums(tau)
S <- matrix(0,nrow = m,ncol = K)

training_fit_hawkes<-nonhomoHak_estimator_eff_revised(training_data_hawkes,
                                                   A, m, K,H,
                                                   window = fit_wind,
                                                   T = train_time,
                                                   dT=0.1,
...
tau_start <- training_fit_hawkes$tau
B_start <- training_fit_hawkes$B
MuA_start <- training_fit_hawkes$MuA
Pi_start <- training_fit_hawkes$Pi
lam_start <- training_fit_hawkes$lam
...
```{r link pred inhom hawkes,results= 'hide'}
time_windows <- seq(train_time,Time, by = 5)
...
for(i in 1:(length(time_windows)-1)) {
  ...
  ### link predict for this data ###
  curr_z <- apply(tau_start,1,which.max)
  baseline = apply(MuA_start,c(1,2),mean)
  ...
  ### then update the model params ####
  current_fit_hawkes <- nonhomoHak_estimator_eff_revised(as.matrix(data_batch),
                                                   A, m, K,H,
                                                   window = fit_wind,
                                                   T = end_time,
                                                   dT=0.1,
  ...
  tau_start <- current_fit_hawkes$tau
  B_start <- current_fit_hawkes$B
  MuA_start <- current_fit_hawkes$MuA
  Pi_start <- current_fit_hawkes$Pi
  lam_start <- current_fit_hawkes$lam
  print(lam_start)
...
```{r inhom hawkes plot one fit}
overall_pred %>% ggplot(aes(end,norm)) + geom_line()
```

Lets repeat this for multiple simulations.


```{r inhom hawkes multiple sims, cache=TRUE,results='hide'}
...
```{r plot these predictions inhom hawkes}
all_preds %>% ggplot(aes(as.factor(end),norm)) + 
  geom_point() + geom_smooth(se = FALSE) +
  #geom_boxplot() +
  labs(x = "Prediction End Time", y = "Norm of Predictions")
```
...
system.time(alltimes_hawkes <- sampleBlockHak(Time, A, Z, Mu_H, B_H,
                                              lam = 1))
...
pars <- c(rep("b",Time/dT),rep("mu",Time/dT))
