...
sbm_bin_ARI <- function(bin_matrix,K, true_Z){
  sbm_bin = BM_bernoulli('SBM',bin_matrix,explore_min = K, explore_max = K,verbosity = 0)
  sbm_bin$estimate()
...
sbm_count_ARI <- function(count_matrix,K,true_Z){
  sbm_count = BM_poisson('SBM',count_matrix, explore_min = K, explore_max = K, verbosity = 0)
  sbm_count$estimate()
