...
Pi <- c(0.3, 0.3, 0.4)
B <- matrix(c(1, 0.5, 0.5, 0.5, 1, .65, 0.75, 0.85, 1.),
            nrow = K,
            ncol = K,
            byrow = T)
...
results <- mainVEM(
  data = sim_data, n = m, Qmin = 3, Qmax = 3, method = "kernel",
  directed = T,
...
Mu_H <- matrix(c(0.6, 0.2, 0.3, 0.1, 1.0, 0.4, 0.5, 0.4, 0.8),
               K, K, byrow = TRUE)
B_H <- matrix(c(0.7, 0.1, 0.1, 0.15, 0.85, 0.2, 0.01, 0.4, 1),
              K, K, byrow = TRUE)
...
results_H <- mainVEM(
  data = sim_data_H, n = m, Qmin = 3, Qmax = 3, method = "kernel",
  directed = T,
  d_part = 1, n_perturb = 1
)
...
system.time(results_H <- online_estimator(alltimes,
                                          A,
                                          m,
                                          K,
                                          T,
...
mean(Pois_P_SBM_ARI)
mean(Pois_B_SBM_ARI)
# performance similar to Poisson SBM when Poisson data.
# our model doesn't use multiple
# restarts, etc
...
sbm_count <- BM_poisson("SBM",
                        A_matrix,
                        explore_min = K,
                        explore_max = K, verbosity = 0)
sbm_count$estimate()
