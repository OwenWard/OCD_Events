...
system.time(results.eff <- nonhomoHak_estimator_eff(alltimes,
                                                    A,
                                                    m,
                                                    K,
                                                    H,
...
itermax <- T / dT
stop_eps <- 0.001
system.time(results.batch <- batch_nonhomoHak_estimator(alltimes,
                                                        A,
                                                        m,
                                                        K,
                                                        H,
...
loglik.eff <- get_loglik_nonhomoHak(alltimes, 0, T, Z,
                                    results.eff$MuA, 
                                    results.eff$B, results.eff$Pi,
                                    A, results.eff$lam,
                                    m, K, H, window)

Z <- apply(results.batch$tau,1,which.max) - 1
loglik.batch <- get_loglik_nonhomoHak(alltimes,
                                      0, T, Z, results.batch$MuA, 
                                      results.batch$B, results.batch$Pi,A,
                                      results.batch$lam,
                                      m, K, H, window)
...
loglik.online <- get_loglik_nonhomoHak(alltimes, 0, T, Z,
                                       results.online$MuA, 
                                       results.online$B,
                                       results.online$Pi,
                                       A, results.online$lam,
                                       m, K, H, window)

Z <- apply(results.batch$tau,1,which.max) - 1
loglik.batch <- get_loglik_nonhomoHak(alltimes, 0, T, Z,
                                      results.batch$MuA, 
                                      results.batch$B,
                                      results.batch$Pi, A,
                                      results.batch$lam,
...
system.time(results.online <- nonhomoPois_estimator(alltimes,
                                                    A,
                                                    m,
                                                    K,
                                                    H,
...
itermax <- T / dT
stop_eps <- 0.001
system.time(results.batch <- batch_nonhomoPois_estimator(alltimes,
                                                         A, m, K,
                                                         H, window, T,
                                                         dT,
                                                         gravity = 0.001,
