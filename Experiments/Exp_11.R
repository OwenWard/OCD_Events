...
dT_vec <- 1
inter_T <- 1
m_vec <- rep(c(500, 1000, 2000, 5000, 10000, 20000, 100000), 2)
...
# for(exp_num in seq_along(K_vec)) {
  ...
  for(sim in 1:no_sims){
    ...
    for(i in 1:m){
      ...
      num_edge <- round(m ^ sparsity) # will be m^alpha for each, 
      # so approx m^{1+alpha} overall
...
saveRDS(results, file = here("Experiments",
                             "exp_results",
                             paste0("exp_11_", m, "_model_", model, ".RDS")))
