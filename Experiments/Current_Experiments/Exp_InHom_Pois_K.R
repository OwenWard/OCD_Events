...
for(sim in 1:no_sims){
  ...
  #### need to choose these in a smart way
  ## simulate from inhomogeneous Poisson instead here
  H <- 2
  MuA <- array(0, c(K, K, H))
  
  MuA[, , 1] <- matrix(0.2, K, K)
  diag(MuA[, , 1]) <- 1.2/1:K  ## instead of 0.8
  MuA[, , 2] <- matrix(0.1, K, K)
  diag(MuA[, , 2]) <- 1.5/K:1
  # 
  ...
  for(curr_wind in 1:10) {
    ...
    init_tau <- matrix(0.01, nrow = m, ncol = K)
    for(i in seq_along(result$est_clust)){
      init_tau[i, result$est_clust[i]] <- 0.9
      init_tau[i,] <- init_tau[i, ]/sum(init_tau[i, ])
    }
